...
```{r, warning = FALSE}
# library()
library(caret) # Used to pre-process the data.
library(ggplot2) # Used to plot any necessary chart / graph.
library(ROCR) # Used to calculate AUC.
library(kknn) # Used to help training the K-nearest Neighbors model.
library(randomForest) # Used to help training the Random Forest model.
library(e1071) # Used to help training the Support Vector Machine model.
library(tree) # Used to build decision tree.
library(adabag) # Used to build adaptive boosting and bagging models
...
library(neuralnet) # Used to help training the neural network.
```
...
for(index in 1:5) {
  ...
  pred = ROCR::prediction(as.numeric(fitted_values), test$Cath)
...
for(index in 1:5) {
  ...
  test_no_cath = test[, !(colnames(test) %in% c("Cath"))]
  boostpredict = predict.boosting(boost, newdata = test_no_cath)
  ...
  #Get AUC
  boostprob = ROCR::prediction(boostpredict$prob[,2], test$Cath)
...
for(index in 1:5) {
  ...
  test_no_cath = test[, !(colnames(test) %in% c("Cath"))]
  baggpredict = predict.bagging(bagg, newdata = test_no_cath)
  #Calculate accuracy of the model
  ...
  #Get AUC
  baggprob = ROCR::prediction(baggpredict$prob[,2], test$Cath)
...
## Section 4: Model Optimisation.

In this section, the best 4 models (Bagging, Boosting, SVM and Random Forest)
training in Section 2 will be further optimized. Additionally, a neural network
will be trained and evaluated in this section using the feature-selected data.  
